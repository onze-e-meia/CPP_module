...
/*   Updated: 2024/12/16 18:57:15 by tforster         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
#ifdef MSG
	const std::string	CLASS_NAME = YLW " - AForm " RST;

	void	AForm::cnstr_msg(std::string class_name, std::string str) const {
...
void	AForm::beSigned(Bureaucrat &bureaucrat) {
	...
	if (_signed) {
		throw SignedException(L_RED "Exception:" RST " Form already signed" ENDL);
	}
	_signed = true;
}

void	AForm::outputStream(std::ostream &os) const {
	os << _name
...
AForm::GradeTooHighException::GradeTooHighException(const std::string &exceptionStr):
...
AForm::GradeTooLowException::GradeTooLowException(const std::string &exceptionStr):
	_exceptionStr(exceptionStr) {}
...
AForm::SignedException::SignedException(const std::string &exceptionStr):
	_exceptionStr(exceptionStr) {}

AForm::SignedException::~SignedException(void)  throw() {}
